...
// la function le compare au secret SUPABASE_NEXUS_API_TOKEN.
 ...
 *
 * Le token attendu est dans Deno.env.SUPABASE_NEXUS_API_TOKEN.
...
export function verifyAuth(req: Request): Response | null {
  const expected = Deno.env.get('SUPABASE_NEXUS_API_TOKEN');
  if (!expected) {
    return errorResponse('server_misconfigured: SUPABASE_NEXUS_API_TOKEN not set', 500);
  }
